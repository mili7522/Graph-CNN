...
def load_sa1_dataset():
    ...
    labels = []
#    categories = []
    with open('../../../Data/2018-06-01-SA1Input-Normalised.csv', 'r') as file:
...
#            labels.append(categories.index(s[-1]))
    ...
    with open('../../../Data/Geography/2018-06-01-Neighbouring_Suburbs_With_Bridges-DropNAN-GCC.csv', 'r') as file:
        ...
        for i, line in enumerate(file):
            ...
            a = keys.index(s[0])
            b = keys.index(s[1])
            adj_mat[a, 0, b] = 0#/(float(s[2]) + 1);  # Inverse of distance
            adj_mat[b, 0, a] = 0#/(float(s[2]) + 1);
...
exp = SingleGraphCNNExperiment('2018-06-01-SA1', '2018-06-01-sa1', SA1Experiment())
