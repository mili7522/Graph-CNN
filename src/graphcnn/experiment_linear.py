...
class GraphCNNExperiment(object):
    ...
    def preprocess_data(self, dataset):
        ...
        self.graph_labels = dataset[2].astype(np.float32)  # Changed from np.int64
...
class SingleGraphCNNExperiment(GraphCNNExperiment):
    ...
    def preprocess_data(self, dataset):
        ...
        self.graph_labels = np.expand_dims(dataset[2].astype(np.float32), axis=0)  # Changed from np.int64
    ...
    def create_loss_function(self):
        ...
        with tf.variable_scope('loss') as scope:
            ...
            # print(self.net.labels.shape)
            # print(self.net.current_V.shape)
            # exit()
            ...
            # self.net.current_V = tf.Print(self.net.current_V, [tf.shape(self.net.labels), tf.shape(self.net.current_V), tf.shape(self.net.current_mask)])
            ...
            # linear_loss = tf.multiply(tf.squeeze(self.net.current_mask), linear_loss)
            # linear_loss = tf.reduce_sum(linear_loss)*inv_sum
