...
def load_sa1_dataset():
    ...
    with open('Data/SA2/2018-05-31-SA2Input-Normalised.csv', 'r') as file:
        for i, line in enumerate(file):
            if i == 0:  # Skip first line (header)
                continue
            s = line[:-1].split(',')  # Last value in line is \n
...
print(exp.graph_vertices.shape)
print(exp.graph_adjacency.shape)
print(exp.graph_labels.shape)
