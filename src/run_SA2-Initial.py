...
def load_sa2_dataset():
    ...
    labels = []
#    categories = []
    with open('datasets/SA2/SA2Comb.csv', 'r') as file:
        for i, line in enumerate(file):
            if i == 0:
                continue
            s = line[:-1].split(',')  # Last value in line is \n
            keys.append(s[0])
            features.extend([int(v) for v in s[1:-1]])  # Last column is the outcome y
...
#            labels.append(categories.index(s[-1]))
    ...
    with open('datasets/SA2/Neighbour Drive Times.csv', 'r') as file:
        adj_mat = np.zeros((len(labels), 1, len(labels)))
        for i, line in enumerate(file):  # Skip first line (header)
            if i == 0:
                continue
            s = line[:-1].split(',')
            a = keys.index(s[0])
...
exp = SingleGraphCNNExperiment('SA2', 'sa2', SA2Experiment())
...
exp.train_idx, exp.test_idx = list(inst.split(np.arange(270)))[0]
